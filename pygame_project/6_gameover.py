# 1. 캐릭터가 공에 맞음 , 2. 모든 공을 없앰, 3. 시간 제한 99초 초과
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    # 나눠진 공 정보
                    small_ball_rect = ball_images[ball_img_idx + 1].get_rect() # 더 작은 다음 단계의 공을 가져와야하기 때문에 idx + 1
                    ...
                    balls.append({    
                        ...
                        "to_x": -3, # x축 이동방향 -3 이면 왼쪽으로, 3 이면 오른쪽으로
                        "to_y": -6, # y축 이동방향
                        "init_spd_y": ball_speed_y[ball_img_idx + 1]}) # y의 최초 속도 다음 작은공 속도가 들어가야하니까 ball_img_idx + 1이 들어감
                    ...
                    balls.append({
                        ...
                        "init_spd_y": ball_speed_y[ball_img_idx + 1]}) # y의 최초 속도 다음 작은공 속도가 들어가야하니까 ball_img_idx + 1이 들어감
        # 2중 for 문 탈출하는 방법
                break # 이걸로 공이 무기와 충돌하면 탈출해서 아래의 지우는 문장으로 가고 싶었는데 2중 for문 오류가 남 = 안쪽 for문 break
        else: # 계속 게임을 진행
            continue # 안쪽 for 문의 조건이 맞지 않으면 continue. 바깥 for 문 계속 수행
        break # 안쪽 for 문에서 break 를 만나면 여기로 진입 가능. 2중 for 문을 한번에 탈출 = 바깥 for 문 break
